...
```{r}
ptb_tobacco <- read_delim('./data/Natality, 2007-2017 (3).txt', delim = '\t') %>% 
  janitor::clean_names() %>% 
  filter(is.na(notes)) %>% 
  select(-notes) %>% 
  mutate(
...
nydata %>% 
  ...
  ggplot(aes(x = mean_temp, y = average_lmp_gestational_age, color = tobacco_use)) + geom_point(alpha = 0.8) + geom_smooth() +
  labs(
    title = "The distribution of average LMP gestational age by temperature, stratified by tobacco use status",
    x = "Daily temperature (C)",
    y = "Average LMP gestational age",
    caption = "Data from the rnoaa package & CDC Wonder database"
  ) +
